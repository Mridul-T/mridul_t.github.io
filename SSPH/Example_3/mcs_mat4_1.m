...
parfor idx = 1:(b - a + 1)
    ...
    results(idx,:,:,:) = reshape(solver_wrapper(X,Y, particles_per_unit, J, i, q), [1, 2, 2500, 101]); % Explicitly match dimensions
end
...
save('results_mcs4_1_test1.mat', 'results_single', '-v7.3');
...
function u0 = cal_u(X, Y, particles_per_unit, J, i)
    ...
    a=normrnd(0.25, 0.05);
    u0_temp(1, :, :) = a * sin(2 * pi * X) .* sin(2 * pi * Y);
    u0_temp(2, :, :) = -a * sin(2 * pi * X) .* sin(2 * pi * Y);
