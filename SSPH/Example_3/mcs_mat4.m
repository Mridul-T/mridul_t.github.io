...
parfor idx = 1:(b - a + 1)
    ...
    results(idx,:,:,:) = reshape(solver_wrapper(X,Y, particles_per_unit, J, i, q), [1, 2, 2500, 301]); % Explicitly match dimensions
end
...
save('results_mcs4_test3.mat', 'results_single', '-v7.3');

% Solver wrapper function
function result = solver_wrapper(X, Y,particles_per_unit, J, i, q)
    u_init = cal_u(X, Y, particles_per_unit, J, i);
    fprintf('Starting the %dth iteration\n', i);
    result = solver(u_init, i); % Assuming solver2 is a MATLAB function
...
function u0 = cal_u(X, Y, particles_per_unit, J, i)
    ...
    u0_temp(1, :, :) = normrnd(0.25, 0.05) * sin(2 * pi * X) .* sin(2 * pi * Y);
    u0_temp(2, :, :) = -normrnd(0.1, 0.01) * sin(2 * pi * X) .* sin(2 * pi * Y);
...
function ut = solver(u_init, i)
    ...
    % Set viscosity with random perturbation
    viscosity = 0.05; % Equivalent to np.random.normal()
