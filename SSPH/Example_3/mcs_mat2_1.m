...
save('results_mcs2_1_test.mat', 'results_single', '-v7.3');
...
function result = solver_wrapper(i, q, u_mean_x, u_mean_y, eigenvalues_x, eigenvectors_x, eigenvalues_y, eigenvectors_y)
    ...
    rng(i);
    wx = randn(1, q);
    result = solver(cal_u(wx, u_mean_x, u_mean_y, eigenvalues_x, eigenvectors_x, eigenvalues_y, eigenvectors_y, q),i);
    disp(["Done with " num2str(i) "th iteration"]);
end

% Function to calculate u(wx, wy)
function val = cal_u(wx, u_mean_x, u_mean_y, eigenvalues_x, eigenvectors_x, eigenvalues_y, eigenvectors_y, q)
    val = zeros(size(u_mean_x));
    val(1, :) = u_mean_x;
    val(2, :) = u_mean_y;
    for i = 1:q
        val(1, :) = val(1, :) + sqrt(eigenvalues_x(i)) * (eigenvectors_x(:, i)' * wx(i));
        val(2, :) = val(2, :) + sqrt(eigenvalues_y(i)) * (eigenvectors_y(:, i)' * wx(i));
    end
end
