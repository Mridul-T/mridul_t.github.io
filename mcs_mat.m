...
u_init = cal_u(X, Y, particles_per_unit, J);
...
parfor idx = 1:(b - a + 1)
    ...
    results(idx,:,:,:) = reshape(solver_wrapper(i, q, u_init), [1, 2, 2500, 301]); % Explicitly match dimensions
end
...
save('results_mcs_test2.mat', 'results_single', '-v7.3');

% Solver wrapper function
function result = solver_wrapper(i, q, u_init)
...
function u0 = cal_u(X, Y, particles_per_unit, J)
...
function ut = solver(u_init, i)
    ...
    viscosity = vis(normrnd(0, 1)); % Equivalent to np.random.normal()
